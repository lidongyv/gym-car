...
# @Last Modified time: 2019-07-27 05:17:35
...
model_p=torch.nn.DataParallel(model_p,device_ids=[6,7])
model_p.cuda(6)
...
def loss_function(recon_x, x, mu, logsigma):
    ...
    BCE = F.mse_loss(recon_x,x,reduction='mean')/recon_x.shape[0]
    ...
    #KLD=torch.max(KLD,torch.ones_like(KLD).cuda(6)*LSIZE*0.5)
    KLD=torch.mean(KLD)
...
def train(epoch):
    ...
    for batch_idx, [data,action,pre] in enumerate(train_loader):
        ...
        vis.line(
            ...
            update='append')
        if batch_idx % 1 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss_c: {:.4f}\tLoss_f: {:.4f}\tLoss_p: {:.4f}'.format(
...
for epoch in range(trained+1, args.epochs + 1):
    ...
    if not args.nosamples:
        print('saving image')
        with torch.no_grad():
            sample = torch.randn(RED_SIZE, LSIZE).cuda(6)
