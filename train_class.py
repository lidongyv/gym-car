...
# @Last Modified time: 2019-08-04 17:55:34
...
from data.loaders_class import RolloutObservationDataset

parser = argparse.ArgumentParser(description='VAE Trainer')
parser.add_argument('--batch-size', type=int, default=8*64, metavar='N',
...
controller=Controller_class(LSIZE,3)
...
vis = visdom.Visdom(env='class_control')
...
def train(epoch,vae_dir,training_sample):
	...
	for batch_idx, [data,action,pre] in enumerate(train_loader):
		...
		#print(action_p[0].shape,action[:,0,0,0].shape)
		loss_steer=action_loss(action_p[0],action[:,0,0,0].long())
		loss_gas=action_loss(action_p[1],action[:,1,0,0].long())
		loss_brake=action_loss(action_p[2],action[:,2,0,0].long())
		loss_a=loss_steer+loss_gas+loss_brake
		#print(action[:,:,0,0])
		...
		action_pr=torch.cat([torch.argmax(action_p[0],dim=1).unsqueeze(-1),torch.argmax(action_p[1],dim=1).unsqueeze(-1), \
					torch.argmax(action_p[2],dim=1).unsqueeze(-1)],dim=1).detach() \
					.view(action.shape[0],3,1,1).expand(action.shape[0],3,action.shape[-1],action.shape[-2]).float()
...
vae_dir = join(args.logdir, 'class')
